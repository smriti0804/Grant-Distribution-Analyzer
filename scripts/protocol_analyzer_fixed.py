#!/usr/bin/env python3
"""
merkl_disperse.py - FIXED VERSION
...
FIXED: Corrected beneficiary reward totaling and unique transaction handling
"""
...
# Disperse/tracing config
STOP_ADDRESS = (
    "0x544cbe6698e2e3b676c76097305bba588defb13a".lower(),
...
_windows_default_csv = r"C:\Users\Smriti\Desktop\llabs\tool\watchdog\MVP\merkl_beneficiaries.csv"
if os.name == "nt" and os.path.isdir(os.path.dirname(_windows_default_csv)):
    MERKL_CSV_PATH = _windows_default_csv
else:
...
def select_transactions_collection_for_protocol(protocol_addr: str):
    """
    Find a collection that contains at least one document where 'from' equals the
    normalized protocol address. Returns the collection name if found, else None.
    """
...
def get_arb_balance(address):
    """Get ARB token balance, with fallback from Alchemy to direct on-chain call."""
    try:
        address = Web3.to_checksum_address(address)
    except Exception:
        return Decimal("0")
    # Try Alchemy
    url = f"https://arb-mainnet.g.alchemy.com/v2/{ALCHEMY_KEY}"
    payload = {"jsonrpc": "2.0", "id": 1,
               "method": "alchemy_getTokenBalances",
               "params": [address, [ARB_ARB_TOKEN]]}
    try:
        resp = requests.post(url, json=payload, timeout=25).json()
        toks = resp.get("result", {}).get("tokenBalances", [])
        if toks:
            bal_hex = toks[0].get("tokenBalance", "0x0")
            bal = Decimal(int(bal_hex, 16)) / Decimal(10 ** 18)
            if bal > 0:
                return bal
    except Exception as e:
        if DEBUG:
            print(f"[DEBUG] Alchemy balance fetch failed for {address}: {e}")

    # Fallback: Direct eth_call to ERC20 balanceOf
    try:
        balance_data = w3.eth.call({
            "to": ARB_ARB_TOKEN,
            "data": "0x70a08231" + address[2:].rjust(64, '0')
        })
        bal = Decimal(int(balance_data.hex(), 16)) / Decimal(10 ** 18)
        return bal
    except Exception as e:
        if DEBUG:
            print(f"[DEBUG] Direct on-chain balance fetch failed for {address}: {e}")
    return Decimal("0")

# ===== Core Traversal (trace_flow) =====
def trace_flow(address,
               parent_incoming_amt=None,
               visited=None,
               is_initial=False,
               intermediaries=None,
               beneficiaries=None,
               holdings=None,
               processed_disperse_payouts=None,
               amount_received_map=None,
               recorded_edges=None,
               beneficiary_unique_txs=None):
    ...
    holdings = holdings or defaultdict(Decimal)
    processed_disperse_payouts = processed_disperse_payouts or set()
    recorded_edges = recorded_edges or set()
    # NEW: Track unique transactions per beneficiary to avoid double counting
    beneficiary_unique_txs = beneficiary_unique_txs or defaultdict(set)

    addr_l = (address or "").lower()
    if not addr_l or addr_l == STOP_ADDRESS:
        return beneficiaries, intermediaries, holdings, amount_received_map, beneficiary_unique_txs
    ...
    if is_initial:
        ...
        for tx in sorted(outgoing_all, key=lambda x: parse_iso(x.get("timeStamp") or "") or datetime.min):
            ...
            visited.add(to_addr)
            sub_ben, intermediaries, holdings, amount_received_map, beneficiary_unique_txs = trace_flow(
                to_addr, amt_sent, visited, False,
                intermediaries, beneficiaries, holdings,
                processed_disperse_payouts,
                amount_received_map,
                recorded_edges,
                beneficiary_unique_txs
            )
    ...
    if not incoming_txs:
        beneficiaries[addr_l] += (parent_incoming_amt or Decimal("0"))
        return beneficiaries, intermediaries, holdings, amount_received_map, beneficiary_unique_txs
    ...
    if is_disperse_contract(addr_l):
        ...
        if DEBUG:
            print(f"[DEBUG] Disperse matched rows for {addr_l}:")
            for r in matched_rows:
                print(r)

        matched_hashes = {row.get("txHash") for row in matched_rows if row.get("txHash")}
        total_incoming_for_matched = sum((incoming_amt_by_hash.get(h, Decimal("0")) for h in matched_hashes), Decimal("0"))
        scale_factor = Decimal(parent_incoming_amt) / total_incoming_for_matched if total_incoming_for_matched and parent_incoming_amt else Decimal("1")
        ...
        for row in matched_rows:
            ...
            if not rec_addr or rec_addr == STOP_ADDRESS:
                continue
                
            # FIXED: Create unique transaction identifier using (timeStamp, txHash) - exclude value to allow same amounts
            unique_tx_key = (timestamp_str, txh_db)
            
            # Skip if this exact transaction has already been processed for this beneficiary
            if unique_tx_key in beneficiary_unique_txs[rec_addr]:
                if DEBUG:
                    print(f"[DEBUG] Skipping duplicate transaction for {rec_addr}: {unique_tx_key}")
                continue
                
            # Mark this unique transaction as processed for this beneficiary
            beneficiary_unique_txs[rec_addr].add(unique_tx_key)
            
            # Old payout key for backward compatibility (can be removed if not needed)
            payout_key = (rec_addr, txh_db)
            processed_disperse_payouts.add(payout_key)

            raw_amt = Decimal(value_str)
            scaled_amt = raw_amt * scale_factor
            beneficiaries[rec_addr] += scaled_amt
            amount_received_map[rec_addr] = amount_received_map.get(rec_addr, Decimal("0")) + raw_amt
        ...
        intermediaries[addr_l] += total_incoming_for_matched
        return beneficiaries, intermediaries, holdings, amount_received_map, beneficiary_unique_txs

    # Normal intermediary - Apply balance limiting logic for non-disperse contracts
    outgoing_after = [tx for tx in outgoing_all if (parse_iso(tx.get("timeStamp") or "") or datetime.min) >= earliest_dt] if earliest_dt else outgoing_all
    outgoing_after_sorted = sorted(outgoing_after, key=lambda x: parse_iso(tx.get("timeStamp") or "") or datetime.min)
    ...
    for tx in outgoing_after_sorted:
        ...
        if cumulative + amt <= total_incoming_amt + Decimal("1e-18"):
    ...
    if unspent > 0:
        holdings[addr_l] = get_arb_balance(addr_l)

    if limited_outgoing and total_outgoing_amt > 0 and (parent_incoming_amt or Decimal("0")) > 0:
        intermediaries[addr_l] += total_incoming_amt
        scale_factor = (Decimal(parent_incoming_amt) / total_outgoing_amt) if total_outgoing_amt and parent_incoming_amt else Decimal("0")
        ...
        for tx in limited_outgoing:
            ...
            scaled_amt = raw_amt_sent * scale_factor

            amount_received_map[to_addr] = raw_amt_sent
            visited.add(to_addr)
            sub_ben, intermediaries, holdings, amount_received_map, beneficiary_unique_txs = trace_flow(
                to_addr, scaled_amt, visited, False,
                intermediaries, beneficiaries, holdings,
                processed_disperse_payouts,
                amount_received_map,
                recorded_edges,
...
def find_beneficiaries(protocol_recipient, out_prefix="arb"):
    ...
    beneficiaries, intermediaries, holdings, amount_received_map, beneficiary_unique_txs = trace_flow(
    ...
    # FIXED: Use beneficiaries dict for amounts, not amount_received_map
    ben_df = pd.DataFrame([
        (addr, str(beneficiaries.get(addr, Decimal("0"))))
        for addr in beneficiaries
    ], columns=["beneficiary_address", "amount_received"])
    ...
    ben_csv = f"{out_prefix}_beneficiaries.csv"
    ben_df.to_csv(ben_csv, index=False)

    pd.DataFrame(sorted(((k, str(v)) for k, v in intermediaries.items()), key=lambda x: Decimal(x[1]), reverse=True),
                 columns=["intermediary_address", "amount_received"]).to_csv(f"{out_prefix}_intermediaries.csv", index=False)

    pd.DataFrame(sorted(((k, str(v)) for k, v in holdings.items()), key=lambda x: Decimal(x[1]), reverse=True),
                 columns=["address", "arb_holdings"]).to_csv(f"{out_prefix}_holdings.csv", index=False)

    print(f"Beneficiaries: {len(beneficiaries)} | Intermediaries: {len(intermediaries)} | Holdings: {len(holdings)}")
    print(f"CSV outputs: {ben_csv}, {out_prefix}_intermediaries.csv, {out_prefix}_holdings.csv")
    
    # DEBUG: Print unique transaction counts per beneficiary
    if DEBUG and beneficiary_unique_txs:
        print(f"\n[DEBUG] Unique transaction counts per beneficiary:")
        for addr, unique_txs in beneficiary_unique_txs.items():
            if addr in beneficiaries:
                print(f"  {addr}: {len(unique_txs)} unique transactions, total amount: {beneficiaries[addr]}")
    ...
    return beneficiaries, intermediaries, holdings

# ------------------------
# Merkl-specific helpers
# ------------------------
...
def fetch_creator_addresses():
    ...
    if selected_transactions_coll is None or not selected_transactions_coll_name:
        print("⚠️ No selected transactions collection; skipping creator discovery")
        return []

    try:
        if not db[selected_transactions_coll_name].find_one({"to": TARGET_TO_ADDRESS}, projection={"_id": 1}):
            print(f"⚠️ Target address {TARGET_TO_ADDRESS} not found in '{selected_transactions_coll_name}'; skipping Merkl fetch")
            return []
    except Exception as e:
        print(f"❌ Error checking target address presence in '{selected_transactions_coll_name}': {e}")
        return []

    print(f"🔍 Looking for creator addresses in '{selected_transactions_coll_name}' where to={TARGET_TO_ADDRESS}...")
    pipeline = [
    ...
    for r in results:
        ...
        if raw_addr:
            ...
            try:
                ...
                print(f" ⚠️ Error checksumming addr {raw_addr}: {e}")
    print(f"✅ Found {len(creators)} creator addresses")
    if DEBUG:
        print(creators)
    return creators
...
def fetch_campaigns(creator_address, refresh: bool = False):
    ...
    if existing and not refresh:
        print(f"✅ Found {len(existing)} campaigns in DB for creator {creator_address}")
        return existing

    print(f"⏳ Fetching campaigns from Merkl API for creator {creator_address} ...")
    ...
    try:
        ...
        print(f"[WARN] Error fetching campaigns: {err}")
        return existing
    ...
    print(f"✅ Stored {len(campaigns)} campaigns in DB")
    return campaigns
...
def fetch_recipients(campaign_id, refresh: bool = False):
    if not refresh:
        ...
        if existing:
            print(f"✅ Found {len(existing)} recipients in DB for campaign {campaign_id}")
            return existing

    print(f"⏳ Fetching recipients from Merkl API for campaign {campaign_id} ...")
    ...
    try:
        ...
        print(f"[ERROR] Could not fetch recipients for {campaign_id}: {err}")
    ...
    print(f"✅ Stored {len(recipients)} recipients in DB for campaign {campaign_id}")
    return recipients
...
def verify_data_integrity(creator_address):
    ...
    print(f"📊 API Total Rows: {api_total_rows}")
    print(f"📊 MongoDB Stored Rows: {stored_rows}")

    if api_total_rows == stored_rows:
        print("✅ PERFECT! All data preserved - no duplicates lost")
    else:
        print(f"⚠️  Data mismatch: {api_total_rows - stored_rows} rows lost")
...
def export_to_csv(selected_campaign_ids=None):
    ...
    print(f"\n📄 Exporting data to {MERKL_CSV_PATH} ...")
    ...
    if not recipients:
        print("⚠️  No recipient data found to export")
        return
    ...
    print(f"✅ Exported {len(beneficiary_totals)} unique beneficiaries to {MERKL_CSV_PATH}")
    merkl_export_performed = True
    try:
        total_sum = sum(beneficiary_totals.values())
        print(f"📊 Total amount distributed: {total_sum:.6f}")
    except Exception:
        pass
...
def clear_existing_data():
    ...
    print(f"✅ Deleted {deleted_count.deleted_count} existing records")
    return deleted_count.deleted_count

# ------------------------
# NEW: Combine beneficiaries CSVs
# ------------------------
def combine_beneficiaries(arb_csv="arb_beneficiaries.csv", merkl_csv=MERKL_CSV_PATH, out_csv="combined_beneficiaries.csv"):
    """
    Combine arb_beneficiaries.csv and merkl_beneficiaries.csv.
    If a beneficiary address appears in both, add its amount_received.
    """
    print("\n🔗 Combining beneficiaries from arb + merkl CSVs ...")
    ...
    def load_csv(path):
        ...
        for _, row in df.iterrows():
            addr = str(row.get("beneficiary_address", "")).lower()
            try:
                amt = Decimal(str(row.get("amount_received", "0")))
            except Exception:
                amt = Decimal("0")
            combined[addr] += amt
    ...
    if disperse_export_performed:
        ...
        print("ℹ️ Skipping ARB CSV in combine because Disperse tracing did not run in this session")
    if merkl_export_performed:
        load_csv(merkl_csv)
    else:
        print("ℹ️ Skipping Merkl CSV in combine because Merkl export did not run in this session")

    rows = [(addr, str(amt)) for addr, amt in combined.items()]
    df_out = pd.DataFrame(rows, columns=["beneficiary_address", "amount_received"])
    df_out["sort_key"] = df_out["amount_received"].apply(lambda v: Decimal(v))
    df_out = df_out.sort_values(by="sort_key", ascending=False).drop(columns=["sort_key"])
    df_out.to_csv(out_csv, index=False)

    print(f"✅ Combined {len(combined)} unique beneficiaries into {out_csv}")
    try:
        total = sum(Decimal(v) for v in df_out["amount_received"])
        print(f"📊 Total combined distributed amount: {total:.6f}")
    except Exception:
        pass
...
# Programmatic API
def compute_beneficiaries_for_protocol(protocol_addr: str) -> pd.DataFrame:
    """
    Runs the merkl (if applicable) and disperse/normal tracing flows for the given
    protocol address, and returns a pandas DataFrame with columns
    [beneficiary_address, amount_received] representing combined results.
    """
...
def main(argv):
    ...
    # Select transactions collection once using the protocol address so both flows align
    protocol_addr = "0x98237513fcB956f63D52074D507970E8Fc4D5e82"
    try:
        set_transactions_collection_by_protocol(protocol_addr)
        if DEBUG:
            print(f"[DEBUG] Using transactions collection (global): {selected_transactions_coll_name}")
    except Exception as e:
        print(f"❌ Could not select transactions collection for {protocol_addr}: {e}")
