...
UPDATED: Fixed beneficiary reward totaling with unique (timeStamp, txHash, value) combination
"""
...
def trace_flow(address,
    ...
    if is_disperse_contract(addr_l):
        ...
        for row in matched_rows:
            ...
            # Create unique transaction identifier using (timeStamp, txHash) - exclude value to allow same amounts
...
def find_beneficiaries(protocol_recipient, out_prefix="arb"):
    ...
    # FIX: Use beneficiaries dict for amounts, not amount_received_map
    ben_df = pd.DataFrame([
...
def fetch_creator_addresses():
    ...
    try:
        if not db[selected_transactions_coll_name].find_one({"to": TARGET_TO_ADDRESS}, projection={"_id": 1}):
            print("[INFO] Target address {} not found in '{}'; skipping Merkl fetch".format(TARGET_TO_ADDRESS, selected_transactions_coll_name))
            return []
    except Exception as e:
        print("[ERROR] Error checking target address presence in '{}': {}".format(selected_transactions_coll_name, e))
        return []

    print("[INFO] Looking for creator addresses in '{}' where to={}...".format(selected_transactions_coll_name, TARGET_TO_ADDRESS))
    pipeline = [
...
def combine_beneficiaries(arb_csv="arb_beneficiaries.csv", merkl_csv=MERKL_CSV_PATH, out_csv="combined_beneficiaries.csv"):
    ...
    if disperse_export_performed:
        ...
        print("[INFO] Skipping ARB CSV in combine because Disperse tracing did not run in this session")
    if merkl_export_performed:
        load_csv(merkl_csv)
    else:
        print("[INFO] Skipping Merkl CSV in combine because Merkl export did not run in this session")
    ...
    print("[INFO] Combined {} unique beneficiaries into {}".format(len(combined), out_csv))
    try:
        total = sum(Decimal(v) for v in df_out["amount_received"])
        print("[INFO] Total combined distributed amount: {:.6f}".format(total))
    except Exception:
        pass
...
def compute_beneficiaries_for_protocol(protocol_addr: str) -> dict:
    ...
    protocol address, and returns a dictionary with beneficiaries, intermediaries, and holdings data.
    """
    ...
    beneficiaries, intermediaries, holdings = find_beneficiaries(protocol_addr, out_prefix="arb")

    # Use in-memory data for API response
    beneficiaries_list = [
        {"beneficiary_address": addr, "amount_received": str(amount)}
        for addr, amount in beneficiaries.items()
    ]
    intermediaries_list = [
        {"intermediary_address": addr, "amount_received": str(amount)}
        for addr, amount in intermediaries.items()
    ]
    holdings_list = [
        {"address": addr, "arb_holdings": str(amount)}
        for addr, amount in holdings.items()
    ]

    # Calculate total distributed, skipping NaN/empty
    total_distributed = "0"
    try:
